...
export function useRoomTimeline({
  ...
  const mapEventToMessage = useCallback(
    (event: MatrixEvent): MessageItem | null => {
      ...
      let videoInfo: { w?: number; h?: number; mimetype?: string } | undefined;
      ...
      if (content.msgtype === MsgType.Text) {
        ...
        // Check for external_url (e.g., Instagram videos)
        const externalUrl = content.external_url;
        if (externalUrl && typeof externalUrl === 'string') {
          // Use external_url as the content so getInstagramUrl can detect it
          contentText = externalUrl;
        } else {
          const mxcUrl = content.file?.url || content.url;
          if (mxcUrl && typeof mxcUrl === 'string') {
            videoUrl =
            ...
            const thumbnailMxc = content.info?.thumbnail_url;
